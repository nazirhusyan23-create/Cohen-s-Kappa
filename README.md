...
1. **Replace the placeholder domain.** Every file uses `https://kappacalc.app/` in
...
2. AdSense reviewers check for: real, useful content (this tool + the ~500-word guide +
   FAQ qualify), a working About/Privacy/Contact page (included), and no placeholder/lorem
   ipsum text (double-check the `[TO-DO]` items above are filled in before you apply).
3. Once approved, add the AdSense site verification script to `<head>` in every HTML
   file, and swap each `<!-- AdSense ad unit goes here -->` comment block inside the
   `.ad-inner` divs in `index.html` for your real `<ins class="adsbygoogle">` unit code.
4. Create an `ads.txt` file at the repo root with the line Google gives you
   (`google.com, pub-XXXXXXXXXXXXXXXX, DIRECT, f08c47fec0942fa0`) — required for AdSense
   to serve ads on your domain.
