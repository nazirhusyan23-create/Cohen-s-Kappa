...
Replace the placeholder domain. Every file uses `https://kappacalc.app/` in
...
`contact.html` — replace `hello@REPLACE-WITH-YOUR-DOMAIN.com` with a real inbox.
`privacy.html` — fill in the "Last updated" date and the analytics disclosure
once you know exactly what scripts (AdSense, analytics, etc.) you're running.
Add a real `og-image.png` (1200×630) if you want rich social previews, and add
...
Steps in order:
Deploy the site (see above) so you have a live URL.
Apply at adsense.google.com with that domain.
AdSense reviewers check for: real, useful content (this tool + the ~500-word guide +
...
Once approved, create your three ad units and paste their slot IDs into `index.html`.
Note: new AdSense accounts are frequently held to a "low traffic" review period.
